...
const handleError = async (response: Response) => {
    if (response.status) {
        ...
        if (
            response.status === ERROR_NOT_LOGGED_CODE &&
            response.url.indexOf(REFRESH_TOKEN_ENDPOINT) > -1
        ) {
            throw new Error(ERRORS[response.status].message, {
                cause: response.status,
            });
        } else if (response.status !== ERROR_NOT_LOGGED_CODE) {
            throw new Error(
                jsonResponse.message || ERRORS[response.status].message,
                { cause: response.status }
            );
        }
    }
...
export const login = async (
    ...
    const response = await post<UserRequest, IAuthResponse>(
...
): Promise<ICallsResponse> => {
    return get<ICallsResponse>(
        `${API_URL}${CALLS_ENDPOINT}?offset=${offset}&limit=${limit}`
    );
};
